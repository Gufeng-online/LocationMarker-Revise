...
from location_marker_revise import constants
from location_marker_revise.storage import LocationStorage, Point, Location
...
def get_coordinate_text(coord: Point, dimension: int, *, color=RColor.green, precision: int = 1):
	def tp_hint(text):
		if config.teleport_hint_on_coordinate:
			text.c(RAction.suggest_command, 'x:{}, y:{}, z:{}, dim:{}'.format(coord.x, coord.y, coord.z, get_dim_key(dimension))).h('点击高亮')
		return text
...
def ifmap(loc:Location):
	if loc.dim==0:
		return RText('overworld')
	elif loc.dim==-1:
		return RText('the_nether')
	elif loc.dim==1:
		return RText('the_end')
	
def add_map(location:Location):
	command = "xaero_waypoint_add:{}'s Location:{}:{}:{}:{}:6:false:0:Internal_{}_waypoints".format(location.name, location.name[0], int(location.pos.x), int(location.pos.y), int(location.pos.z),ifmap(location))
	return RText('[+X]', RColor.gold).h('§6Xaeros Minimap§r: 点击添加路径点').c(RAction.run_command, command)
...
def tp(dimension: int,loc:Point):
	return RText('[tp]',RColor.gold).h('点击传送到此').c(RAction.run_command,'/execute in {} run tp {} {} {}'.format(get_dim_key(dimension), loc.x, loc.y, loc.z))
...
def print_location(location: Location, printer: Callable[[RTextBase], Any], *, show_list_symbol: bool):
	...
	text = RTextList(
		...
		add_map(location),
		' ',
		tp(location.dim,location.pos),
		'  ',
		set_dimension_text(location),
...
@new_thread('LocationMarker-Revise')
...
def show_location_detail(source: CommandSource, name):
	...
	if loc is not None:
		...
		source.reply(RTextList('坐标: ', RText('[{},{},{}]').format(int(loc.pos.x),int(loc.pos.y),int(loc.pos.z))))
